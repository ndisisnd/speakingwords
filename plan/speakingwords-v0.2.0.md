...
- **Upstream prevention (`SessionStart` injection)** — a `SessionStart` hook emits the active style rules (voice + conciseness level, scoped "applies to user-facing prose only") as `additionalContext`, once per session. Rationale: no hook event can intercept reply text pre-display — `MessageDisplay` is display-only, so Stop is inherently lint-after — which makes prevention the only way to cut bounces. The trade is favourable: one ~200–400 token block early in context (prompt-cache-friendly, not per-prompt) against a full reply regeneration per avoided bounce. The Stop hook stays as the enforcement backstop; injection failure changes nothing (fail-open, A26). Both agents — Codex parity confirmed and shipped, one byte-identical script wired through each agent's own config file (§8).
...
- **Codex injection parity (W2)** — RESOLVED (2026-08-14): parity confirmed, so Codex ships the injector too. Codex CLI supports `SessionStart` in `.codex/hooks.json` (project) and `~/.codex/hooks.json` (profile), using the same lifecycle event names, the same stdin payload (`source`, `session_id`, `transcript_path`, `cwd`, `hook_event_name`, `model`) and the same `{"hookSpecificOutput": {"hookEventName": "SessionStart", "additionalContext": …}}` stdout contract as Claude Code. `hook_session.py` is therefore installed byte-identically for both agents and only the config file naming it differs. Sources: [Codex hooks docs](https://developers.openai.com/codex/hooks), openai/codex#13014 (closed, implemented). Caveat, accepted: on Codex 0.130.0 a bare `codex` launch that auto-restores the previous thread emits no `SessionStart` (openai/codex#24228) — that session gets no block, the Stop backstop is unaffected, which is the fail-open behaviour A26 already requires. Documented in the README's Codex specifics. Version floor unchanged: a Codex below `v0.124.0` still takes the audit-only `notify` path and gets no injector, because `notify` has no way to put text into the model's context.
