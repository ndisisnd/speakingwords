 ...
 *
 * @returns {{intents: Array<{kind:'ban'|'allow', phrase:string, clause:string}>, unparsed: string[]}}
 */
...
function run(hint, options = {}) {
  ...
  const { intents, unparsed } = parseHint(hint);
  if (intents.length === 0) {
  ...
  if (result.changes.length === 0) {
    lines.push('No rule change to make.');
  ...
  let backups;
  try {
    backups = backupAll([lexiconPath, ...memoryTargets]);
  } catch (err) {
    writeErr(`\n${err.message}\n`);
    return 1;
  }

  fs.writeFileSync(lexiconPath, result.text, 'utf8');
