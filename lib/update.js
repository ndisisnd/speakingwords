...
//   §5  Where a marker block is installed — memory mode, and `both` — it is
//       re-rendered afterwards, so the rules written into CLAUDE.md and the
//       rules in the lexicon never disagree. At `both` that is one pass over
//       two artefacts, not two commands.
//
...
The hint is plain English. Say what you want less of, or more of:
  ...
  speakingwords update "simplified technical english"   # switch register
  ...
  simplified technical english · ste                ->  switches to the ste register
  slack register · back to slack                    ->  switches back

The conciseness level is low or high, and moves one step at a time.
The register is slack or ste, and a hint sets it outright rather than stepping.
...
// A register hint sets the installed register outright. There are two
// registers and no order between them, so unlike the conciseness dial there is
// nothing to step through — the hint names the one it wants.
//
// Checked before everything else for the same reason the level hints are:
// "less contractions" and "simplified technical english" would otherwise read
// as requests to add strip rules for the literal words in them.
const STE_RE = /\b(?:simplified technical english|ste register|ste|asd-?ste-?100|technical english)\b/i;
const SLACK_RE = /\b(?:slack register|back to slack|slack|normal register|plain register)\b/i;
 ...
 *            levels: Array<{step: 1|-1, clause: string}>,
 *            registers: Array<{value: 'slack'|'ste', clause: string}>,
 *            unparsed: string[]}}
 */
function parseHint(hint) {
  ...
  for (const clause of clauses) {
    // Register first, then level: both look like ban/allow clauses and are not.
    // `ste` is checked before `slack` so "back to slack from ste" is read as a
    // move to slack only if slack is the one named as the destination — the
    // two patterns are disjoint on every phrasing the help text documents.
    if (STE_RE.test(clause) && !SLACK_RE.test(clause)) {
      registers.push({ value: 'ste', clause });
      continue;
    }
    if (SLACK_RE.test(clause)) {
      registers.push({ value: 'slack', clause });
      continue;
    }
  ...
  return { intents, levels, registers, unparsed };
}
...
function run(hint, options = {}) {
  ...
  const { intents, levels, registers, unparsed } = parseHint(hint);
  if (intents.length === 0 && levels.length === 0 && registers.length === 0) {
  ...
  // A register move is the same kind of edit as a level move: it lives in
  // pref.json, not the lexicon, so it needs an install to write to. The last
  // register named in the hint wins — a hint that says both is a user changing
  // their mind mid-sentence, and the end of the sentence is the answer.
  if (registers.length) {
    if (!found) {
      writeErr(
        '\nThere is no speakingwords install to change the register on.\n'
        + 'Run `speakingwords init` first — nothing was edited.\n'
      );
      return 1;
    }
    const wanted = registers[registers.length - 1].value;
    const current = pref.register(found.pref);
    if (wanted === current) {
      levelNote.push(`Register is already ${current}.`);
    } else {
      nextRegister = wanted;
      prevRegister = current;
    }
  }
  ...
  const lines = [''];
  // Which installs carry a block to keep in step. Memory mode has one; `both`
  // has one as well, alongside the hook that reads the lexicon. At `both` a
  // single update pass therefore moves both artefacts — they derive from the
  // same rule file, so letting one lag would put two different rule sets in
  // front of the same reply (plan v0.3.0 W6).
  const hasBlock = Boolean(found && found.pref
    && (found.pref.mode === 'memory' || found.pref.mode === 'both'));

  if (result.changes.length === 0 && !nextLevel && !nextRegister) {
  ...
  const prefTargets = (nextLevel || nextRegister) && found && fs.existsSync(found.path)
    ? [found.path]
    : [];
  ...
  if (nextLevel || nextRegister) {
    // writePref preserves every key it was not handed, so this moves the axis
    // that changed and nothing else — including keys written by a version that
    // is not this one.
    const moved = { ...found.pref };
    if (nextLevel) moved.conciseness = nextLevel;
    if (nextRegister) moved.register = nextRegister;
    pref.writePref(moved);
    if (nextLevel) found.pref.conciseness = nextLevel;
    if (nextRegister) found.pref.register = nextRegister;
  }

  const rerendered = [];
  if (hasBlock) {
    // The block is re-rendered from the pref as it stands now, so a register
    // move lands in CLAUDE.md in the same pass that moved pref.json. The two
    // must never state different registers.
    const block = memory.renderBlock({
      voice: found.pref.voice || 'terse',
      conciseness: pref.conciseness(found.pref),
      register: pref.register(found.pref),
  ...
  if (nextRegister) {
    lines.push(`Register is now ${nextRegister} (was ${prevRegister}).`);
    lines.push(`  recorded in ${found.path}`);
    if (nextRegister === 'ste') {
      lines.push('  Sentences are capped at 25 words and contractions bounce.');
      lines.push('  STE-inspired, not conformant STE: writing rules only, no word list.');
    }
    lines.push('');
  }
  ...
  } else if (found && found.pref && (found.pref.mode === 'hook' || found.pref.mode === 'both')) {
    lines.push('');
