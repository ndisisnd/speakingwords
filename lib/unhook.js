...
function describeRemoval(result) {
  ...
  if (result.kind === 'claude') {
    const map = {
      removed: `removed the Stop and SessionStart hook entries from ${result.path}`,
    ...
    const hookMap = {
      removed: `removed the Stop and SessionStart hook entries from ${h.path}`,
...
async function run(options = {}) {
  ...
  for (const agent of agents) {
    ...
    if (agent === 'claude') {
      ...
      warn.push(`    SessionStart hook entry in ${hooks.settingsPath(scope, cwd)}`);
    } else {
      warn.push(`  ${label}`);
      warn.push(`    Stop hook entry in ${hooks.codexHooksPath(scope, cwd)}`);
      warn.push(`    SessionStart hook entry in ${hooks.codexHooksPath(scope, cwd)}`);
