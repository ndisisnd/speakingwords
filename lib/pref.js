...
const KNOWN_KEYS = ['agents', 'mode', 'scope', 'voice', 'version', 'conciseness'];

// The three conciseness levels, and what a util does with anything else.
// `null` means "never chosen" — only a 0.1.0 install can be in that state, and
// every reader treats it as `high`, which is the band 0.1.0 already behaved in
// (plan §8). Preserving behaviour beats guessing at an intent nobody stated.
const LEVELS = ['low', 'med', 'high'];
const DEFAULT_LEVEL = 'med';       // what init suggests to a new install
...
function conciseness(pref) {
  const value = pref && pref.conciseness;
  return LEVELS.includes(value) ? value : FALLBACK_LEVEL;
}
...
function defaults() {
  return { agents: [], mode: null, scope: null, voice: null, version: null, conciseness: null };
}
