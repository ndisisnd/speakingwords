...
//
// Two levels ship. `med` is a recognised legacy value from 0.2.0 development
// and normalises to `high`, whose behaviour and band it became — so an old
// pref.json renders the `high` line rather than throwing or falling silent.
const CONCISENESS_BULLETS = {
  low: 'Conciseness is low: keep the prose, cut only decoration — filler, restatement, stacked hedges.',
  high: 'Conciseness is high: every sentence earns its place. Keep explanations, cut elaborations.',
};

// Level-gated language rule (lang-function-over-inventory). It ships as its own
// line rather than folded into the high conciseness line because it is a
// language rule, not a cut: it changes what a report says, not how much of it
// survives. Only rendered at the level it is active at, so a low install never
// carries an instruction it is not meant to follow.
const CONCISENESS_EXTRA_BULLETS = {
  high: 'Report what a change does, not the parts it is made of. Keep the count and a pointer — a file, a table, a diff — and drop the roll call. Numbers, paths, code blocks and caveats always stay.',
};

// Legacy level values a reader still understands, mapped to what they became.
// Kept in step with lib/pref.js LEGACY_LEVELS.
const LEGACY_LEVELS = { med: 'high' };

// The register line, one per register. This is a SWAP, not an addition: the
// register bullet is always line 1 of the block, and the STE line replaces the
// Slack-DM line rather than joining it (A30). The two say opposite things about
// contractions, so a block carrying both would ask for a reply nobody can
// write — and the 9-line budget has no room for a second register line anyway.
//
// The STE line is written in the register it prescribes: short sentences, no
// contractions, imperative instructions, articles kept. An instruction that
// breaks its own rule teaches the wrong thing by example, and this line is read
// by the model on every single reply.
//
// STE-inspired, never conformant. The writing rules are implemented; the
// ASD-STE100 approved-word dictionary is not shipped, reproduced or
// approximated, here or anywhere else in this tree.
const REGISTER_BULLETS = {
  slack: 'Write like a colleague in a Slack DM. Short sentences, everyday words, contractions where they read naturally. Technical terms stay — it is the grammar around them that stays simple, never the vocabulary of the domain.',
  ste: 'Write in Simplified Technical English (STE-inspired). Use short sentences of 25 words or fewer. Do not use contractions. Use the active voice. Give an instruction as an imperative. Give one instruction in one sentence. Keep the articles. Keep technical terms exactly as they are.',
};
...
//   v0.2.0 P11 — the spare line went to lang-function-over-inventory, which is
//                only rendered at the most aggressive level. So `high` renders
//                9 lines exactly and `low` renders 8.
//   v0.2.0 P14 — the dial dropped to two positions and `med` became `high`, so
//                the gated line moved with it. Same arithmetic, one level fewer.
//   v0.3.0 P16 — the register arrived and cost nothing: line 1 is the register
//                line, and `ste` swaps its text for the STE one. A swap, not an
//                append, so `high` still renders 9 lines and `low` still 8 at
//                either register (A30).
function baseBullets(phrases, register = 'slack') {
  return [
    REGISTER_BULLETS[register] || REGISTER_BULLETS.slack,
 ...
 * @param {'low'|'high'} [options.conciseness]  defaults to `high`, the level a
 *                                        0.1.0 install already behaved at.
 *                                        `med` is a legacy value and reads as
 *                                        `high`; so does anything unknown.
 * @param {'slack'|'ste'} [options.register]   defaults to `slack`, the register
 *                                        every pre-0.3.0 install behaved at.
 *                                        Anything unknown reads as `slack` too.
...
function renderBlock(options = {}) {
  const { voice, conciseness, register, version = '0.1.0', bullets, lexiconPath } = options;
  // A shipped register wins; anything else — including a missing key — is
  // `slack`, which is what every install before 0.3.0 already behaved as.
  const reg = REGISTER_BULLETS[register] ? register : 'slack';
  // A shipped level wins; a legacy value normalises to what it became; anything
  // else — including a missing key — is `high`.
  const level = CONCISENESS_BULLETS[conciseness]
    ? conciseness
    : (LEGACY_LEVELS[conciseness] || 'high');
  ...
  if (bullets) {
    ...
    lines = baseBullets(readStripPhrases(lexiconPath), reg)
      .concat(VOICE_BULLETS[voice], CONCISENESS_BULLETS[level]);
    if (CONCISENESS_EXTRA_BULLETS[level]) lines.push(CONCISENESS_EXTRA_BULLETS[level]);
  }
  ...
  const header = `<!-- speakingwords v${version} · memory mode · voice: ${voice} · conciseness: ${level} · register: ${reg} · managed block, edits here are overwritten -->`;
