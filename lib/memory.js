...
// line rather than folded into the med conciseness line because it is a language
// rule, not a cut: it changes what a report says, not how much of it survives.
// Only rendered at the level it is active at, so a low or high install never
// carries an instruction it is not meant to follow.
const CONCISENESS_EXTRA_BULLETS = {
  med: 'Report what a change does, not the parts it is made of. Keep the count and a pointer — a file, a table, a diff — and drop the roll call. Numbers, paths, code blocks and caveats always stay.',
};
...
//                only rendered at `med`. So `med` renders 9 lines exactly and
//                `low` and `high` still render 8.
