...
//   2. Add exactly one Stop-hook entry to the right config file — Claude Code's
...
function codexHookCommand(skillRoot) {
  return `python3 ${path.join(skillRoot, ...CODEX_HOOK_SCRIPT.split('/'))}`;
}
...
function installSkillCore(agent = 'claude', explicitRoot) {
  ...
  // The hooks are invoked as `python3 <path>`, but make them executable anyway
  // so a user can run them by hand while debugging.
  for (const rel of [HOOK_SCRIPT, CODEX_HOOK_SCRIPT, CODEX_NOTIFY_SCRIPT]) {
    try {
 ...
 */
function addHookEntry(events, command) {
 ...
 */
function removeHookEntry(events) {
...
function installClaudeHook({ scope, cwd = process.cwd(), skillRoot } = {}) {
  ...
  return { path: file, command, action: actionFor(text, next, found) };
}

/**
 * Add (or refresh) the Stop-hook entry in the chosen Codex hooks.json.
 *
 ...
 *
 * @returns {{path: string, command: string, action: 'created'|'added'|'updated'|'unchanged', trustRequired: true}}
 */
function installCodexHook({ scope, cwd = process.cwd(), skillRoot } = {}) {
  ...
  return {
    path: file,
    command,
    action: actionFor(text, next, found),
...
/**
 * Remove all Codex wiring: the hooks.json entry and the notify line alike.
 *
