 ...
 *
 * @returns {{path: string, command: string, sessionCommand: string, action: 'created'|'added'|'updated'|'unchanged', trustRequired: true}}
 */
function installCodexHook({ scope, cwd = process.cwd(), skillRoot } = {}) {
  ...
  const foundSession = addHookEntry(data, sessionCommand, SESSION_EVENT);
  const next = writeSettings(file, data, format);
