 ...
 * the rules up front so there is less to enforce. The injector is deliberately
 * Claude Code only — Codex has no confirmed pre-reply context channel, so its
 * adapter ships lint-after only (plan §8).
 *
 * @returns {{path: string, command: string, sessionCommand: string, action: 'created'|'added'|'updated'|'unchanged'}}
 */
function installClaudeHook({ scope, cwd = process.cwd(), skillRoot } = {}) {
  ...
  const sessionCommand = sessionHookCommand(root);
  ...
  const foundSession = addHookEntry(data.hooks, sessionCommand, SESSION_EVENT);
